...
import { VERSION, CARD_NAME, EDITOR_NAME, DEFAULTS, DEFAULT_LINK_SPEED, LED_MODES, SPEED_FORMATS } from './constants.js';
...
class PulseSwitchCard extends HTMLElement {
  ...
  /** Set of port rx_entity IDs already warned about for failed tx-entity inference; prevents log spam on every hass tick. @type {Set<string>} */
  _warnedTxInference = new Set();
  ...
  setConfig(config) {
    ...
    if (config.led_mode !== undefined && !LED_MODES.includes(config.led_mode)) {
      warn(
        `led_mode "${config.led_mode}" is not one of ${LED_MODES.join(' / ')}. Falling back to "${DEFAULTS.led_mode}".`,
      );
    }
    if (config.speed_format !== undefined && !SPEED_FORMATS.includes(config.speed_format)) {
      warn(
        `speed_format "${config.speed_format}" is not one of ${SPEED_FORMATS.join(' / ')}. Falling back to "${DEFAULTS.speed_format}".`,
      );
    }

    /* Destroy old LEDs before replacing config so their RAF loops detach. */
    ...
    this._config = {
      ...
      led_mode: LED_MODES.includes(config.led_mode) ? config.led_mode : DEFAULTS.led_mode,
      speed_format: SPEED_FORMATS.includes(config.speed_format) ? config.speed_format : DEFAULTS.speed_format,
  ...
  set hass(hass) {
    ...
    for (const port of this._config.ports) {
      ...
      const txEntityId = port.tx_entity ?? deriveTxEntityId(port.entity);
      const txState = txEntityId ? hass.states[txEntityId] : null;
  ...
  _fullRender() {
    ...
    const compactClass = cfg.layout === 'compact' ? ' ps-compact' : '';
    const entityRowClass = cfg.entity_row ? ' entity-row' : '';

    let html = '';
    if (cfg.title) {
      html += `<div class="ps-panel-title">${escapeHtml(cfg.title)}</div>`;
    }
    html += `<div class="ps-port-grid${compactClass}${entityRowClass}" style="--ps-columns:${sanitizeCssValue(columns)};--ps-columns-narrow:${sanitizeCssValue(narrowColumns)}">`;
    const hideUnavailable = cfg.hide_unavailable === true;
    ...
    // eslint-disable-next-line no-unsanitized/property -- inputs pre-escaped by _renderPortTile
    tpl.innerHTML = fullHtml;
    this._shadow.appendChild(tpl.content.cloneNode(true));

    this._destroyAllLeds();
    this._portElements = {};
    const tiles = this._shadow.querySelectorAll('.ps-port-tile');
    for (const tile of tiles) {
      const entityId = /** @type {HTMLElement} */ (tile).dataset.entity;
      if (!entityId) continue;

      const linkLed = /** @type {HTMLElement} */ (tile.querySelector('.ps-port-led.ps-link'));
      const actLedEl = /** @type {HTMLElement} */ (tile.querySelector('.ps-port-led.ps-activity'));
      const label = /** @type {HTMLElement} */ (tile.querySelector('.ps-port-label'));
      const speed = /** @type {HTMLElement|null} */ (tile.querySelector('.ps-port-speed'));
      const pins = /** @type {HTMLElement[]} */ ([...tile.querySelectorAll('.ps-port-pin')]);
  ...
  _renderPortTile(port, index) {
    ...
    if (portType === 'rj45') {
      const pinsHtml = Array.from({ length: 8 }, () => '<div class="ps-port-pin"></div>').join('');
      bodyContent = `
        <div class="ps-port-socket"><div class="ps-port-pins">${pinsHtml}</div></div>
        <div class="ps-port-tab"></div>`;
    } else if (portType === 'sfp') {
      bodyContent = `
        <div class="ps-port-cage"><div class="ps-port-cage-edge"></div></div>`;
    } else {
      const icon = resolveVirtualIcon(port);
      bodyContent = `
        <div class="ps-port-virtual-icon"><ha-icon icon="${escapeHtml(icon)}"></ha-icon></div>`;
    }

    const ariaLabel = `${escapeHtml(labelText)}: loading`;
    // Body class is the slug-prefixed `ps-{portType}` so initial render
    // matches what _updatePort would toggle on. Without the prefix, the
    // first paint emits an unprefixed `rj45` / `sfp` / `virtual` and any
    // CSS keyed off `.ps-rj45` only kicks in once _updatePort runs.
    const portTypeClass = `ps-${portType}`;

    return `
      <div class="ps-port-tile" data-entity="${escapeHtml(port.entity)}" data-index="${index}"
           tabindex="0" role="status" aria-label="${ariaLabel}">
        <div class="ps-port-body ${portTypeClass}">
          <div class="ps-port-led ps-link ps-disconnected"></div>
          <div class="ps-port-led ps-activity ps-idle"></div>
          ${bodyContent}
        </div>
        <div class="ps-port-label">${escapeHtml(labelText)}</div>
        ${showSpeed ? '<div class="ps-port-speed"></div>' : ''}
      </div>`;
  }
  ...
  _updatePort(port) {
    ...
    /* classList toggle (not className=) so unrelated classes like .has-ripple
       from attachRipple survive state changes. ps- prefix on the class, bare
       string on the state value being compared. */
    const stateToClass = {
      connected: 'ps-connected',
      disconnected: 'ps-disconnected',
      disabled: 'ps-disabled',
      unavailable: 'ps-unavailable',
    };
    for (const [state, cls] of Object.entries(stateToClass)) {
      els.tile.classList.toggle(cls, state === ps.linkState);
    }

    const body = els.tile.querySelector('.ps-port-body');
    if (body) {
      const portTypeToClass = { rj45: 'ps-rj45', sfp: 'ps-sfp', virtual: 'ps-virtual' };
      for (const [type, cls] of Object.entries(portTypeToClass)) {
        body.classList.toggle(cls, type === ps.portType);
      }
      for (const [state, cls] of Object.entries(stateToClass)) {
        body.classList.toggle(cls, state === ps.linkState);
      }
    }

    for (const [state, cls] of Object.entries(stateToClass)) {
      els.linkLed.classList.toggle(cls, state === ps.linkState);
    }

    const isActive = ps.totalSpeed > 0;
    els.actLed.classList.toggle('ps-active', isActive);
    els.actLed.classList.toggle('ps-idle', !isActive);
    ...
    if (led) {
      ...
      if (ps.totalSpeed <= 0 || ps.linkState !== 'connected') {
        ...
        /* Default 'realistic' mode — PPS-driven pulse-stretch. */
        led.update(ps.pps);
      }
    }

    const pinClass = ps.linkState === 'connected' ? 'ps-port-pin' : 'ps-port-pin ps-off';
  ...
  _resolvePortState(port) {
    ...
    // Resolve TX entity: explicit `tx_entity` override wins, else infer
    // by replacing `_rx_speed` suffix. Inference returns null when the
    // pattern doesn't match — warn once per port so the user can spot
    // the mismatch instead of staring at an empty speed display.
    const txEntityId = port.tx_entity ?? deriveTxEntityId(port.entity);
    if (cfg.show_speed && !txEntityId && !this._warnedTxInference.has(port.entity)) {
      warn(
        `show_speed: cannot infer TX entity from "${port.entity}" (expected suffix "_rx_speed"). Set "tx_entity" explicitly on this port, or speed will display rx-only.`,
      );
      this._warnedTxInference.add(port.entity);
    }
    const txState = txEntityId ? hass.states[txEntityId] : null;
    if (cfg.show_speed && txEntityId && !txState && !this._warnedTxInference.has(txEntityId)) {
      warn(
        `show_speed: TX entity "${txEntityId}" not found in hass.states. Speed will display rx-only.`,
      );
      this._warnedTxInference.add(txEntityId);
    }
    ...
    /* utilization is consumed only in 'scaled' led_mode. */
  ...
  _computePps(port, totalSpeed) {
    ...
    /* Priority 1 — real packet count delta when rx_packets / tx_packets entities
       are configured. Falls through to bandwidth estimation when first sample
       (no prior to delta against) or when no packet counters are configured. */
    if (port.rx_packets || port.tx_packets) {
      ...
      if (!isNaN(rxPkts) || !isNaN(txPkts)) {
        ...
        /* First sample stored — fall through to estimation while we wait for
           the next sample to delta against. */
        this._prevPackets[port.entity] = { rx: isNaN(rxPkts) ? 0 : rxPkts, tx: isNaN(txPkts) ? 0 : txPkts, timestamp: now };
      }
    }
  ...
  _cacheStates() {
    ...
    for (const port of this._config.ports) {
      ...
      const txEntityId = port.tx_entity ?? deriveTxEntityId(port.entity);
      const txState = txEntityId ? this._hass.states[txEntityId] : null;
      if (rxState) {
  ...
  connectedCallback() {
    /* When the card is removed and re-attached, _portElements is empty (cleared
       in disconnectedCallback). Detect that here and fire a full re-render. */
  ...
  static async getConfigElement() {
    await import('./pulse-switch-card-editor.js');
