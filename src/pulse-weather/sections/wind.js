...
export function renderWind({ hass, config, discovery, weatherEntity }) {
  ...
  // Saturation ring — when wind / gust exceeds SPEED_CAP the arrow
  // and gust cone clamp at the outer radius, hiding how much further
  // the real value has gone. Render a dashed ring so users see the
  // rose has saturated.
  const isSaturated = displaySpeed > SPEED_CAP || gusts > SPEED_CAP;
  if (isSaturated) {
    parts.push(svgEl('circle', {
      cx: ROSE_CX, cy: ROSE_CY, r: ROSE_MAX_R + 2,
      fill: 'none', stroke: '#ff453a', 'stroke-width': 1.5,
  ...
  /* Tier-coloured wash layered over the cartouche sky baseline. */
  ...
  const washBg = withSkyBaseline(`radial-gradient(ellipse at 50% 50%, ${tierColor}${glowAlpha} 0%, ${tierColor}${innerAlpha} 40%, transparent 85%)`);

  /* Brand mark doubles as the speed/gust toggle — outline = speed, filled
     disc = gust. aria-label reflects the toggle's purpose so screen readers
     announce it as a control, not decoration. */
  const variant = brandMarkVariant(weatherEntity?.state || 'sunny', false);
  const inGustMode = speedMode === 'gust';
  // The kicker reads "wind" in speed mode and "wind · gust" when toggled
  // so the user can tell which value the rose centre is displaying without
  // glancing at a tab pill.
  const kickerText = inGustMode ? 'wind · gust' : 'wind';
  ...
  return renderSectionShell({
    sectionClass: 'pw-wind-v2',
    ariaLabel: 'Wind compass and speed',
    brandVariant: variant,
    kicker: kickerText,
    sectionStyle: `background: ${sanitizeCssValue(washBg)}`,
    body,
    proAction: 'wind-mode',
    proInitial: inGustMode,
    proAriaLabel: 'Toggle speed and gust mode',
