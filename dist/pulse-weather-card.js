    ...
    <div class="${h}" data-pro="${z}" data-pro-action="${d}"${f} role="region" aria-label="${g(t)}">
      ...
      <div class="pw-content">
        ...
        ${S}
          ...
          </div>`})():"",U=h.unit||"m",se=h.value!==null?(()=>{let O=Za(h.value,D);return`
      ...
      </div>`}let jr="",qr="",Gr="",lt="",la=o?.hourly||[];if(la.length>0){let M=gt(la,new Date).slice(0,12),q=M.some(H=>Number(H.precipitation_probability)>0),G=Lo(R,se,ge,O,_e,Z,be,ae);if(R>0)lt=`
      <div class="pw-weather-summary" style="position:relative; z-index:2">
        <span style="font-size:14px">\u{1F327}</span> ${G}
      </div>`;else{let H=ar({now:new Date,cloudCoverNow:de?de.total:0,conditionNow:p,slots:M,windSpeed:h,uvIndex:A,stabilityState:Oe});lt=`
      <div class="pw-weather-summary" style="position:relative; z-index:2">
        ${g(H.sentence)}
      </div>`}if(q){let H=M.map(ye=>{let B=Math.min(100,Math.max(0,Number(ye.precipitation_probability)||0)),re=Number(ye.snowfall)||0,ke=(Number(ye.rain)||0)+(Number(ye.showers)||0),Ke=re>0;if(Ke&&ke>0){let no=re+ke,ua=Math.round(B*re/no),io=B-ua;return`<div class="pulse-precip-slot"><div class="pulse-precip-fill" style="height:${Number(io)}%"></div><div class="pulse-precip-fill pw-precip-snow" style="height:${Number(ua)}%"></div></div>`}return`<div class="pulse-precip-slot"><div class="${Ke?"pulse-precip-fill pw-precip-snow":"pulse-precip-fill"}" style="height: ${B}%"></div></div>`}).join(""),j=M[Math.floor(M.length/2)]?`+${Math.floor(M.length/2)}h`:"";jr=`
      ...
      </div>`}if(M.some(H=>Number(H.cape)>300)){let j=Ro(M,200,24),ye=j.peakIndex/Math.max(M.length-1,1)*200,B=j.peakValue>=1e3?`${(j.peakValue/1e3).toFixed(1)}k`:String(Math.round(j.peakValue)),re=M[j.peakIndex]?.datetime?new Date(M[j.peakIndex].datetime):null,ke=re?Ze(re):"",Ke=Je("pw-cape-grad");qr=`
      ...
      </div>`}M.some(H=>H.dew_point!==void 0&&H.dew_point!==null)&&(Gr=`
      <div style="position: relative; z-index: 2; margin: 2px var(--pulse-space-card-wide) 0;">
        ...
        <div class="pw-comfort-bar">${M.map($e=>{let j=Number($e.dew_point)||0;return`<div class="pw-comfort-slot" style="background:${k(Ba(j))}"></div>`}).join("")}</div>
      </div>`)}let En=Me(m),Nn=de!==null?Ha(de.total):"",Rn=Eo(f),Ln=`${g(Math.round(_))} ${g(T)}${oa?` <span style="color:${k(na)}">${g(oa)}</span>`:""}`,In=`${g(Math.round(A))}${Pr?` <span style="opacity:0.5">/ ${g(String(Math.round(pe)))}</span>`:""}`;function Xr(){return Ze(new Date)}let Yr=Wa(p),Kr=te(p,c),Qr=t?.location_label||"Now",ca=it?it.replace(/<[^>]*>/g,"").trim():"",Zr=lt?lt.replace(/<[^>]*>/g,"").trim():"",Jr=de?Math.round(de.total)+"%":"--",eo=`
        <div class="pw-loc-stamp">${g(Qr)} \xB7 ${g(Xr())}</div>
        <div class="pw-hero-block">
          ${y.display(Math.round(u),{suffix:S})}
          ${y.gloss(`${Yr}${ca?" \xB7 "+ca:""}`)}
        </div>`,to=y.statsRow([y.stat(`${Math.round(E)}/${Math.round(P)}\xB0`,"range"),y.stat(`${Math.round(h)}${Rt?` / ${Math.round(x)}`:""} ${ta}`,Rt?"wind / gust":"wind"),y.stat(Math.round(A),"uv"),y.stat(Jr,"cloud")],{columns:4}),ct=(o?.daily||[]).slice(0,7),Lt="";if(ct.length>=2){let M=ct.map(B=>Number(B.temperature)||0),q=ct.map(B=>Number(B.templow??B.temperature)||0),G=Math.min(...q),xe=Math.max(...M),ne=Math.max(xe-G,1),H=B=>B/(ct.length-1)*320,$e=B=>38-(B-G)/ne*34-2,j=M.map((B,re)=>`${re===0?"M":"L"}${H(re).toFixed(1)},${$e(B).toFixed(1)}`).join(" ");Lt=`<svg viewBox="0 0 320 38" preserveAspectRatio="none">
      ...
      <div class="pw-fc-row${D?" is-today":""}">
      ...
      </div>`}).join(""),{condition:v,isNight:x}=ht(e,o,a),b=te(v,x),$=Io(c),_=`
        <div class="pw-fc-list" role="list">
          ${h}
        </div>`;return Y({sectionClass:"pw-forecast-v2",ariaLabel:"Seven-day forecast",brandVariant:b,kicker:"the week ahead",narrative:$,body:_,pro:t?.pro!==!1})}var je=240,fe=je/2,he=je/2,at=90,ir=.12,_t=6,$t=.5,sr=.25,zo=.3,Do=6,lr=.7,Oo=.2,Po=4,Ho=3.3,Bo=15,Wo=35,Uo=.08,Vo=.2,jo=[{l:"N",a:0},{l:"NE",a:45},{l:"E",a:90},{l:"SE",a:135},{l:"S",a:180},{l:"SW",a:225},{l:"W",a:270},{l:"NW",a:315}];function cr(e){return(e-90)*Math.PI/180}function qo(e){let t=e*Math.PI/180;return{x:Math.sin(t),y:-Math.cos(t)}}function Te(e,t){return{x:fe+t*Math.cos(e),y:he+t*Math.sin(e)}}function Go(e,t,a,r){let o=Math.min(e/We,1)*at,i=Te(t-sr,o),n=Te(t+sr,o),l=a>zo?"pw-gust-sweep":"",p=Math.max(.6,2.5-a*2).toFixed(2),c=(.15+a*.3).toFixed(2),s=`transform-origin:${fe}px ${he}px;--gust-dur:${p}s;--gust-peak:${c}`;return ie("path",{class:l,d:`M${fe},${he} L${i.x.toFixed(1)},${i.y.toFixed(1)} L${n.x.toFixed(1)},${n.y.toFixed(1)} Z`,fill:r,opacity:.15,style:s})}function Xo(e,t,a,r){let o=Math.min(e/We,1)*at,i=Te(t,o),n=Te(t-ir,o*.7),l=Te(t+ir,o*.7),p={x:i.x-_t*Math.cos(t-$t),y:i.y-_t*Math.sin(t-$t)},c={x:i.x-_t*Math.cos(t+$t),y:i.y-_t*Math.sin(t+$t)},s=a*Do,d=Math.max(.4,4-a*3.6),u=s>Oo?`animation:pw-arrowSway ${d.toFixed(2)}s ease-in-out infinite;--sway-fwd:${(s*lr).toFixed(2)}deg;--sway-back:${(-s*(1-lr)).toFixed(2)}deg`:"",m=`transform-origin:${fe}px ${he}px;filter:drop-shadow(0 0 6px ${Se(r,.38)});${u}`,f=ie("path",{d:`M${fe},${he} L${n.x.toFixed(1)},${n.y.toFixed(1)} L${i.x.toFixed(1)},${i.y.toFixed(1)} L${l.x.toFixed(1)},${l.y.toFixed(1)} Z`,fill:r,opacity:.5}),h=ie("line",{x1:fe,y1:he,x2:i.x.toFixed(1),y2:i.y.toFixed(1),stroke:r,"stroke-width":2.5,"stroke-linecap":"round"}),v=ie("path",{d:`M${i.x.toFixed(1)},${i.y.toFixed(1)} L${p.x.toFixed(1)},${p.y.toFixed(1)} L${c.x.toFixed(1)},${c.y.toFixed(1)} Z`,fill:r});return ie("g",{style:m},f+h+v)}function pr({hass:e,config:t,discovery:a,weatherEntity:r}){let o=r.attributes,i=a.atmosCe,n=t.speed_mode||"speed",l=(V,O)=>ft(e,i,o,V,O),p=l("wind_speed","wind_speed"),c=l("wind_gusts","wind_gust_speed")||l("wind_gusts","wind_gusts")||p,s=l("wind_direction","wind_bearing"),u=(i.wind_speed?e.states[i.wind_speed]:null)?.attributes?.unit_of_measurement||o.wind_speed_unit||"km/h",m=n==="gust"?c:p,f=Math.min(p/We,1),h=wt(m),v=jt(m),x=Be[v]||Be.calm,b=bt(s),$=cr(s),_=qo(s),S=Math.round(f*ga),z=Po-f*Ho,T=Uo+f*Vo,C=Bo+f*Wo,N=-_.y,D=_.x,W=[];for(let V=0;V<S;V++){let O=(Math.random()-.5)*400,Z=-120-Math.random()*60,ae=420+Math.random()*60,pe=(C*(.6+Math.random()*.8)).toFixed(1),ve=(z+Math.random()*z*.5).toFixed(2),Ne=(Math.random()*3).toFixed(2),Re=(T*(.5+Math.random()*.5)).toFixed(3),Le=(Z*_.x+O*N).toFixed(1),J=(Z*_.y+O*D).toFixed(1),Ie=(ae*_.x+O*N).toFixed(1),ze=(ae*_.y+O*D).toFixed(1);W.push(`<div class="pw-streak" style="left:50%;top:50%;width:${pe}px;height:1px;transform:rotate(${s-90}deg);animation-duration:${ve}s;animation-delay:${Ne}s;opacity:${Re};background:linear-gradient(to right, transparent, ${Se(x,.27)}, transparent);--sx:${Le}px;--sy:${J}px;--ex:${Ie}px;--ey:${ze}px"></div>`)}let E=[];for(let V=1;V<=3;V++)E.push(ie("circle",{cx:fe,cy:he,r:V*30,fill:"none",stroke:x,"stroke-width":.5,opacity:.08+V*.02}));for(let V of jo){let O=cr(V.a),Z=Te(O,20),ae=Te(O,at+4),pe=Te(O,at+16),ve=V.l.length===1;E.push(ie("line",{x1:Z.x.toFixed(1),y1:Z.y.toFixed(1),x2:ae.x.toFixed(1),y2:ae.y.toFixed(1),stroke:x,"stroke-width":.5,opacity:.12})),E.push(ie("text",{x:pe.x.toFixed(1),y:pe.y.toFixed(1),"text-anchor":"middle","dominant-baseline":"central",fill:ve?"rgba(255,255,255,0.7)":"rgba(255,255,255,0.3)","font-size":ve?11:9,"font-weight":ve?600:400},g(V.l)))}E.push(Go(c,$,f,x)),E.push(Xo(m,$,f,x)),(m>We||c>We)&&E.push(ie("circle",{cx:fe,cy:he,r:at+2,fill:"none",stroke:"#ff453a","stroke-width":1.5,"stroke-dasharray":"4 4",opacity:.7})),E.push(ie("circle",{cx:fe,cy:he,r:18,fill:x,opacity:.06})),E.push(ie("circle",{cx:fe,cy:he,r:3,fill:x}));let w=f>Ot?(f-Ot)*5:0,A=Math.max(.15,.4-f*.25),R="";w>.1&&(R=`--dx:${(w*_.x).toFixed(2)}px;--dy:${(w*_.y).toFixed(2)}px;animation:pw-roseShake ${A.toFixed(2)}s ease-in-out infinite`);let U=Math.round(12+f*50).toString(16).padStart(2,"0"),se=Math.round(6+f*25).toString(16).padStart(2,"0"),ge=Oa(`radial-gradient(ellipse at 50% 50%, ${x}${U} 0%, ${x}${se} 40%, transparent 85%)`),we=te(r?.state||"sunny",!1),_e=n==="gust",Fe=_e?"wind \xB7 gust":"wind",be=`
        ...
        ${D}
        ${W}
        ${E}`;return Y({sectionClass:"pw-air-quality-v2",ariaLabel:"Air quality index",brandVariant:S,kicker:N,body:P,proAction:"aqi-scale",proInitial:z,proAriaLabel:"Toggle EU and US AQI scale",pro:t.pro!==!1})}function Qo(e){let t=(e%ee+ee)%ee;return Fa[Math.floor(t/ee*8)%8]}function Zo(e){return(e%ee+ee)%ee<ee/2}var Kt=864e5;function le(e){return!e||isNaN(e.getTime())?"--:--":e.toLocaleTimeString([],{hour:"2-digit",minute:"2-digit",hour12:!1})}function Jo(e,t,a,r,o){if(r==null)return`<circle cx="${e.toFixed(1)}" cy="${t.toFixed(1)}" r="${a}" fill="#c8d0e0"/>`;let i=Math.max(0,Math.min(1,r/100)),n=o!=null?Zo(o):!0,l=`<circle cx="${e.toFixed(1)}" cy="${t.toFixed(1)}" r="${a}" fill="#2a2a3a"/>`;if(i>=.99)l+=`<circle cx="${e.toFixed(1)}" cy="${t.toFixed(1)}" r="${a}" fill="#e8e0d0"/>`;else if(i>.01){let p=t-a,c=t+a,s=(a*Math.abs(2*i-1)).toFixed(1),d=i>.5?1:0,u;n?u=`M${e.toFixed(1)},${p.toFixed(1)} A${a},${a} 0 0 1 ${e.toFixed(1)},${c.toFixed(1)} A${s},${a} 0 0 ${d} ${e.toFixed(1)},${p.toFixed(1)} Z`:u=`M${e.toFixed(1)},${p.toFixed(1)} A${a},${a} 0 0 0 ${e.toFixed(1)},${c.toFixed(1)} A${s},${a} 0 0 ${1-d} ${e.toFixed(1)},${p.toFixed(1)} Z`,l+=`<path d="${u}" fill="#e8e0d0"/>`}return l}var kt="#050510",mr="#0a1226",fr="#172b4f",hr="#1f3d6e",gr="#3a6fa8",wr="#d97a3a",br="#ff9f0a",vr="#ffc560",xr="#7eaedb",yr="#2c5a8e",_r="#ffd60a";function en(e,t,a,r,o,i,n,l,p,c){let s=new Date(e.getTime());s.setHours(0,0,0,0);let d=e.getTime(),u=t.getTime(),m=(a??new Date(d-35*6e4)).getTime(),f=(r??new Date(d-30*6e4)).getTime(),h=(o??new Date(d-6*6e4)).getTime(),v=(i??new Date(d+30*6e4)).getTime(),x=(n??new Date(u-30*6e4)).getTime(),b=(l??new Date(u+6*6e4)).getTime(),$=(p??new Date(u+30*6e4)).getTime(),_=(c??new Date(u+35*6e4)).getTime(),S=(d+u)/2,z=P=>Math.max(0,Math.min(100,P)),T=P=>z((P-s.getTime())/Kt*100),C=(P,w)=>T((P+w)/2),N=Math.min(60,(u-d)/8)*6e4,D=S-N,W=S+N;return`linear-gradient(to right, ${[`${kt} 0%`,`${kt} ${T(m).toFixed(2)}%`,`${mr} ${C(m,f).toFixed(2)}%`,`${fr} ${T(f).toFixed(2)}%`,`${hr} ${C(f,h).toFixed(2)}%`,`${gr} ${T(h).toFixed(2)}%`,`${wr} ${C(h,d).toFixed(2)}%`,`${br} ${T(d).toFixed(2)}%`,`${vr} ${C(d,v).toFixed(2)}%`,`${xr} ${T(v).toFixed(2)}%`,`${yr} ${C(v,D).toFixed(2)}%`,`${_r} ${T(D).toFixed(2)}%`,`${_r} ${T(W).toFixed(2)}%`,`${yr} ${C(W,x).toFixed(2)}%`,`${xr} ${T(x).toFixed(2)}%`,`${vr} ${C(x,u).toFixed(2)}%`,`${br} ${T(u).toFixed(2)}%`,`${wr} ${C(u,b).toFixed(2)}%`,`${gr} ${T(b).toFixed(2)}%`,`${hr} ${C(b,$).toFixed(2)}%`,`${fr} ${T($).toFixed(2)}%`,`${mr} ${C($,_).toFixed(2)}%`,`${kt} ${T(_).toFixed(2)}%`,`${kt} 100%`].join(", ")})`}function $r(e){if(!e||isNaN(e)||e<=0)return"--";let t=Math.round(e/6e4),a=Math.floor(t/60),r=t%60;return`${a}h ${String(r).padStart(2,"0")}m`}function kr({hass:e,config:t,discovery:a}){let r=new Date,o=a.sunEntityId?e.states[a.sunEntityId]:null;if(!o)return null;let i=o.attributes||{},n=i.next_rising||i.sunrise,l=i.next_setting||i.sunset;if(!n||!l)return null;let p=new Date(r);p.setHours(12,0,0,0);let c=K(new Date(n),p),s=K(new Date(l),p);if(!c||!s||isNaN(c.getTime())||isNaN(s.getTime()))return null;let d=a.atmosCe,u=ue=>d[ue]?new Date(String(e.states[d[ue]]?.state)):null,m=K(u("golden_hour_morning_start"),c),f=K(u("golden_hour_morning_end"),c),h=K(u("blue_hour_morning_start"),c),v=K(u("golden_hour_evening_start"),s),x=K(u("golden_hour_evening_end"),s),b=K(u("blue_hour_evening_end"),s),$=i.next_dawn,_=i.next_dusk,S=$?K(new Date($),c):null,z=_?K(new Date(_),s):null,T=d.moon_phase?e.states[d.moon_phase]:null,C=T?.attributes||{},N=C.moon_age!==void 0?Number(C.moon_age):null,D=C.illumination!==void 0?Number(C.illumination):null,W=N!==null?Qo(N):T?.state||null,E=K(u("moonrise"),r),P=K(u("moonset"),r),w=r.getTime()>=c.getTime()&&r.getTime()<=s.getTime(),A=le(c),R=le(s),U=(c.getTime()+s.getTime())/2,se=le(new Date(U)),ge=s.getTime()-c.getTime(),we=Kt-ge,_e=$r(ge),Fe=$r(we),be=w?"Daylight":"Night",V=w?_e:Fe,O=new Date(c.getTime());O.setHours(0,0,0,0);let Z=Math.max(0,Math.min(100,(r.getTime()-O.getTime())/Kt*100)),ae=en(c,s,S,h,m,f,v,x,b,z),ve=`
      ...
      </div>`,Ne=D!=null?Math.round(D):null,Re=W||"Moon",Le=Jo(24,24,22,D,N),J=Ne!==null?`<span class="pw-astro-moon-illum">${g(`${Ne}%`)}</span> illuminated`:"",Ie=N!=null?`${N.toFixed(1)}d old`:"",ze=N!=null?(()=>{let ue=ee/2,nt=(N%ee+ee)%ee;return`${(nt<=ue?ue-nt:ee-nt+ue).toFixed(1)}d to full`})():"",Mt=[J,Ie,ze].filter(Boolean).join(" \xB7 "),Tt=`
